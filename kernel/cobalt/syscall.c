			...
			struct mq_attr __user *u_attr)
{
	...
	assoc = pse51_assoc_lookup(&pse51_queues()->uqds, (u_long)uqd);
	if (assoc == NULL)
		return -EBADF;
		      ...
		      int fd)
{
	...
	err = pse51_assoc_insert(&pse51_queues()->ufds,
				 &assoc->assoc, fd);
	if (err) {
		xnfree(assoc);
		close(kfd);
...
static int __shm_close(int fd)
{
	...
	assoc = pse51_assoc_remove(&pse51_queues()->ufds, fd);
	if (assoc == NULL)
		return -EBADF;
...
static int __ftruncate(int fd, off_t len)
{
	...
	assoc = pse51_assoc_lookup(&pse51_queues()->ufds, fd);
	if (assoc == NULL)
		return -EBADF;
